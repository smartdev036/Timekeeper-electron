...
class LogTimes {
    private static async RemoveCombinationsIfEmpty(db: Database, $log_date: string) {
        db.run(`
    ...
    static async Add(log_time: LogTimes, isClosing: boolean = false) {
        ...
        const stmnt = db.prepare(
            `INSERT INTO log_times
                (position_id, controller_id, trainee_controller_id, log_date, start_time)
            VALUES
                ($position_id, $controller_id, $trainee_controller_id, $log_date, $start_time)
            `
        )
    ...
    static async Delete(position_id: number, log_date: string, start_time: number) {
        const db = await Open()
    ...
    static async GetAllPositionsStatusValues(log_date: string, until_time?: number) {
        ...
        const result = db.exec(
            ...
            FROM (
                SELECT * from log_times
                WHERE log_date = $log_date
                ${typeof until_time === "undefined" ? "" : `AND position_id = ${until_time}`}
    ...
    static async GetReport(type: ReportTypes, start_date: string, end_date: string) {
        const db = await Open()
        ...
        FROM (
            SELECT
                ${type === "protime" ? protime_select_controller_id : "log_times.controller_id"},
                log_times.position_id,
                MIN(n.start_time) - log_times.start_time AS duration
            FROM log_times
            ...
            ${type === "protime" ? "" : "AND log_times.trainee_controller_id != 0"}
