...
class LogTimes {
	...
	static async GetPositionByControllerId(
		controller_id: number,
		log_date: string,
		until_time?: number
	) {
		...
		>(result).filter(
			(row) =>
				...
				row.trainee_controller_id === controller_id
		);

		let position_ids = position_status.map((row) => row.position_id);

		let combination_center = AsObject<{ position_id: number }>(
			db.exec(
				`
            SELECT
                position_id
		...
		if (position_ids.length === 1) {
			...
			t_position_id = position_ids.filter((position_id) =>
				combination_center.includes(position_id)
			)[0];
		}
		let position_pos = AsObject<{ position: number }>(
			db.exec(
				`
            SELECT
                position
