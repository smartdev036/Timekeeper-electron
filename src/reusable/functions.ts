...
export const fromSerializedTime = (TotalSeconds: number, toNormalTime: boolean = false) => {
    const [TotalMinutes, seconds]  = [Math.floor(TotalSeconds / 60), Math.floor(TotalSeconds % 60)]
...
export const isEmpty = (...values: string[]) => values.some(val => val.trim() === "")