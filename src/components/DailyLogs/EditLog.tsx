...
const EditLog = ({ position, trigger, log }: EditLogProps) => {
	...
	const { value: controller_value } = useAsyncRefresh(
		() => DB.Controller.GetAll(),
		[DB, counter]
	);
	const controllers: OptionType[] =
	...
	const trainee = controllers.find(
		({ id }) => id === log.trainee_controller_id
	);
