...
import AppSettingProvider, {
	useAppSetting,
} from "./context/AppSettingProvider";

import Logo from "./static/logo.png";
import DaylightIcon from "./static/daylight.png";
...
const RouterLink = ({ to, label }: RouterLinkProps) => {
	return (
		<NavLink
			to={to}
			className={({ isActive }) =>
				clsx(styles.RouterLink, isActive ? styles.active : "")
			}
		>
			{label}
		</NavLink>
			...
			<Container className={styles.Main}>
				<Container noMargin={true} className={styles.Header}>
					...
					<div className={styles.Time}>
						<FiClock />
						<span>
							{localTimeDateString} | {localTime.formatToString() + " "}
						</span>
						{isdaylight ? (
							<img src={DaylightIcon} className={styles.DaylightIcon} />
						) : (
							""
						)}
						<span className={styles.zSpan}>{"| " + utcTime.formatToZ()} </span>
					</div>
				</Container>
